        ...
        <div className="max-w-4xl mx-auto pb-10">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
                <div className="flex items-center gap-3">
                    <h1 className="text-2xl sm:text-3xl font-extrabold text-[#020B1C] tracking-tight uppercase">Notificaciones</h1>
                    {unreadCount > 0 && (
                        <span className="bg-[#ED123A] text-white text-xs font-bold rounded-full h-6 w-6 flex items-center justify-center shadow-sm">
                            {unreadCount}
                        </span>
                    )}
                </div>
                <button
                    ...
                    className="bg-white text-gray-700 px-4 py-2 flex items-center justify-center rounded-[14px] hover:bg-gray-50 transition-colors border border-gray-200 shadow-sm text-sm font-bold uppercase"
                >
                    <FontAwesomeIcon icon={faCheckDouble} className="mr-2 text-[#020B1C]" />
                    Marcar todas leídas
                </button>
            </div>
            ...
            {sortedNotifications.length > 0 ? (
                    ...
                    <div className="bg-white rounded-[14px] border border-gray-200 overflow-hidden shadow-sm">
                        <ul className="divide-y divide-gray-100">
                            {currentNotifications.map(notification => (
                                <li
                                    key={notification.id}
                                    className={`flex items-center gap-4 p-4 sm:p-5 transition-colors ${!notification.isRead ? 'bg-red-50/50' : 'bg-white hover:bg-gray-50'
                                        } ${isClickable(notification) ? 'cursor-pointer' : ''}`}
                                    onClick={() => handleNotificationClick(notification)}
                                >
                                    <div className="mt-1">
                                        <FontAwesomeIcon icon={faBell} className={`w-5 h-5 ${!notification.isRead ? 'text-[#ED123A]' : 'text-gray-400'}`} />
                                    </div>
                                    <div className="flex-1">
                                        <p className={`text-sm ${!notification.isRead ? 'text-[#020B1C] font-bold' : 'text-gray-600 font-medium'}`}>
                                    ...
                                    <div className="flex-shrink-0">
                                        <button
                                            ...
                                            className="p-2 text-gray-400 hover:text-[#ED123A] transition-colors rounded hover:bg-gray-100/80"
                    ...
                    {totalPages > 1 && (
                        <div className="flex justify-between items-center mt-8">
                            <button
                                ...
                                className="px-4 py-2 rounded-[14px] bg-white text-gray-700 disabled:opacity-50 disabled:cursor-not-allowed hover:bg-gray-50 transition-colors flex items-center gap-2 font-bold text-sm border border-gray-200 shadow-sm"
                            >
                ...
                <div className="text-center py-20 px-4 bg-white rounded-[14px] border border-gray-200 border-dashed">
                    ...
                    <h3 className="text-lg font-bold text-[#020B1C]">No tienes notificaciones</h3>
