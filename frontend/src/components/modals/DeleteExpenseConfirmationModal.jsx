        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4 animate-fade-in-up">
            <div className="bg-component-bg backdrop-blur-lg rounded-2xl shadow-2xl w-full max-w-md border border-border-color">
                <div className="p-8 text-center">
                    ...
                    <h2 className="text-2xl font-bold text-text-primary">¿Eliminar Gasto?</h2>
                    <p className="text-text-secondary mt-2">
                        Estás a punto de eliminar permanentemente un gasto.
                    </p>
                    
                    <div className="mt-4 p-4 rounded-xl">
                         <p className="font-bold text-text-primary text-2xl">
                             {new Intl.NumberFormat('es-ES', { style: 'currency', currency: 'EUR' }).format(expense.amount)}
                         </p>
                         {expense.description && (
                             <p className="text-sm text-text-secondary mt-2 italic truncate px-2" title={expense.description}>
                                 "{expense.description}"
                             </p>
                         )}
                    </div>
